...
# This variable will give us the full path to this current script i.e., .../src/Residual_Plot_Script.py
...
# We want to write one script that generates plot residual values for each of the three ML models
# In order to do this we will have to define a specific function called plot_residuals()

def plot_residuals(y_true, y_pred, model_name, output_name):

    # The purpose of this function is to create a residual scatter plot for a given ML model based on the inputs
    ...
    # the true target values as the x-axis and residuals, the residuals for a given model as the y-axis.
    # alphas is just to determine how opaque each individual point is so, 0.6 makes them slightly see-through.

    plt.axhline(0, color="red", linestyle="--")
    # This draws a horizontal line at y=0
    # This is a reference point for prediction
    # if the residual is at 0, it's a perfect prediction
    # If the residual is above 0, it's an underprediction
    # If the residual is below 0, it's an overprediction
...
plot_residuals(y_test, lr_pred, "Linear Regression", "LR_residuals.png")
plot_residuals(y_test, rf_pred, "Random Forest", "RF_residuals.png")
plot_residuals(y_test, gb_pred, "Gradient Boosting", "GB_residuals.png")

# for each ML model, we execute the plot_residuals() function with the appropriate inputs
