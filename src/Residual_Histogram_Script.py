...
# This variable will give us the full path to this current script i.e., .../src/Residual_Histogram_Script.py
...
# We want to write one script that generates a residual histogram for each of the three ML models
# In order to do this we will have to define a specific function called plot_residual_histogram()

def plot_residual_histogram(y_true, y_pred, model_name, output_name):

    # The purpose of this function is to create a histogram of the residual values for a given ML model based on the inputs
    ...
    # We make a residuals variable that computes the residual for each player.
    # The residual is the true target value minus the predicted value for a given ML Model
    ...
    # So this creates a histogram using the residuals variable defined earlier
    # bins=30 means we want 30 bars in our histogram
    # We want a black outline around each bar and an alpha of 0.7 in order to make the bars a bit see-through

    plt.axvline(0, color="red", linestyle="--")
    # This draws a vertical line at x=0
    # This is a reference point for prediction
    # if the residual is at 0, it's a perfect prediction
    # If the residual is to the right of 0, it's an underprediction
    # If the residual is to the left of 0, it's an overprediction

    plt.title(f"Residual Histogram ({model_name})")
    plt.xlabel("Residual (Actual - Predicted)")
    plt.ylabel("Frequency")
    # Here we are simply adding the title and axes labels
...
plot_residual_histogram(y_test, lr_pred, "Linear Regression", "LR_residual_hist.png")
plot_residual_histogram(y_test, rf_pred, "Random Forest", "RF_residual_hist.png")
plot_residual_histogram(y_test, gb_pred, "Gradient Boosting", "GB_residual_hist.png")

# for each ML model, we execute the plot_residual_histogram() function with the appropriate inputs
